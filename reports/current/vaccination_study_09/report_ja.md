...
大規模 dataset で generic T Cell はほぼ残っていませんが、Blood Cell fallback が大きく残ります。B-cell label は比較的安定しており、effector-memory T は強い支持がある subcluster に限定されています。
...
- この dataset の主な確認対象: 大規模 dataset で generic T Cell はほぼ残っていませんが、Blood Cell fallback が大きく残ります。B-cell label は比較的安定しており、effector-memory T は強い支持がある subcluster に限定されています。
