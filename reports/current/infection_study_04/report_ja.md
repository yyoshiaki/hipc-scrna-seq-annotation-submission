...
B と T の両方に中程度の parent label が残っています。reference source が割れる部分で過剰に fine label を付けない設計ですが、naive/memory B と T-state の境界が主な確認対象です。
...
- この dataset の主な確認対象: B と T の両方に中程度の parent label が残っています。reference source が割れる部分で過剰に fine label を付けない設計ですが、naive/memory B と T-state の境界が主な確認対象です。
