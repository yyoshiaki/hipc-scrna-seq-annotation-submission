...
vaccination_study_09 は 139,960 cells / 19,141 portal genes の dataset です。親ラベルまたは Blood Cell に残った割合は 0.5%、doublet は 579 cells、低 confidence は 1,286 cells でした。注意すべき marker gene 欠損は Plasma_ASC(warning: JCHAIN) です。 大規模な T/B dataset で、screfmap は B/CD4T の補助として広く利用できます。 subcluster evidence では CD4 Naive / T Central Memory: 64,895 cells; Classical Monocyte: 25,752 cells; Naive B Cell: 11,614 cells; CD8 Cytotoxic / T Effector Memory: 10,513 cells; NK Cell: 10,322 cells; Non-Classical Monocyte: 4,365 cells が主要な構造です。全細胞に近い coverage で最も broad lineage に沿った source は Marker score (broad concordance 97.8%) で、相対的に不一致が目立つ source は Azimuth PBMC L3 (36.5%) です。 screfmap は適用範囲を B/CD4T に限定すると coverage 40.0%、broad concordance 98.9% でした。 v14 marker registry audit は、marker gene list をそのまま全細胞で競わせるのではなく、broad lineage、applicable lineage、key-marker support の順に制限した場合に marker evidence がどう変わるかを見る診断です。単純 winner では Eosinophil 101,081 cells、Platelet 166 cells のような rare/artifact label が出やすい一方、gate 後は Eosinophil 40 cells、Platelet 87 cells に抑制されます。これは final label を marker score だけで置き換えるためではなく、fine label を受け入れる条件と confidence cap を決めるための evidence audit です。 最も gate 後の未割当が多い lineage は Other_lineage (29.2%) です。
