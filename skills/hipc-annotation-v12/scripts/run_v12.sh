...
HIPC_V12_MANIFEST="${HIPC_V12_MANIFEST:-configs/v12_manifest.example.tsv}"
REPORT_LANGUAGES="${REPORT_LANGUAGES:-en}"
HIPC_V12_VALIDATE_ONLY="${HIPC_V12_VALIDATE_ONLY:-0}"

cd "$REPO_ROOT"
if [[ "$HIPC_V12_VALIDATE_ONLY" != "1" ]]; then
  "$PYTHON_BIN" scripts/pipeline/hipc_annotate_v12.py   --config configs/v12_pipeline.json   --manifest "$HIPC_V12_MANIFEST"   --out "$HIPC_V12_OUT"   --report-languages "$REPORT_LANGUAGES"
fi
